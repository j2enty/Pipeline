...
#
# code-reviewer·verifier·critic(모드 A/B/C) 에이전트 파일의 불변식을 단언한다:
#   (A) frontmatter 필수 필드(name·model) 존재 + name 일치.
#   (B) 반환 JSON 계약 보존 — code-reviewer·verifier·critic(모드C) 의 필드명·severity
#       값이 원본(.tmpl)과 1:1 (skill 파싱 강결합이라 한 글자도 못 바꿈).
...
for f in "$CODE_REVIEWER" "$VERIFIER" "$CRITIC"; do
