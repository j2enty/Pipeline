...
- **R11** (도입 · 2026-05-03) Docs 커밋 트리거에 `critic verdict=concerns` 추가 — 기존 10-a 트리거 (a)(b)(c) 는 개별 verdict (escalated/request_changes) 또는 SIGINT/혼합 상태만 감지. 개별 모두 approved + critic concerns 케이스가 빠져 있어 cross-area finding 이 영구 보존되지 않는 문제 발견 (관찰 사례: 2026-05-03 `m2-scaffolding` — 두 PR APPROVE 인데 critic 이 blocker 1건 + major 2건 짚음). 해결: 트리거에 (d) `aggregate.criticFindings 가 비어있지 않으면` 추가. critic finding 이 있으면 `<docs-context-dir>/<slug>-status.md` 자동 push 해서 M3 작업자가 진입 시 컨텍스트 로드 가능하게. (#47: 최초 도입 시 `aggregate.criticVerdict == "concerns"` 로 적었으나 그 필드는 상태 파일 어디에도 기록되지 않는 死문이라 트리거가 영영 안 걸렸고, `concerns` 한정이라 blocker 케이스도 놓쳤음 → `criticFindings` 비어있지 않음 으로 정정.)
