...
8-c 의 parent 코멘트 렌더와 **별개로**, critic 이 반환한 finding 을 상태 파일에 구조화 기록한다(코멘트는 휘발성, 상태 파일은 영구 보존·후속 추적용):
...
- `aggregate.verdict` 는 **critic 종합 verdict**(`pass`/`concerns`/`blocker`)이며 `.github/workflows/critic.yml` 이 이 값을 읽어 머지 분기한다(allowlist 밖 값은 fail-closed 로 머지 차단). 개별 PR verdict(`prs.<area>.verdict`: `approved`/`request_changes`)와는 별개다 — 혼동 금지.
