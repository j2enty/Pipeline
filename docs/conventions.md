...
| `codex-review-model` | `claude-commands` (직속) | string | 빈 값 | `/review` codex 2차 리뷰(`codex review`)에 주입할 모델명. 빈값이면 미주입=codex 자체 기본 모델(본체 기본 빈값, 회귀 없음). 계정 종류에 따라 가용 모델이 달라 프로젝트가 더 강한 모델을 쓸 수 있게 config 주입. 값이 있을 때만 `-c model=<값>` 플래그 부착. SKILL 리더 전용 |
| `codex-review-reasoning-effort` | `claude-commands` (직속) | string | 빈 값 | `/review` codex 2차 리뷰 모델의 reasoning effort(예: `high`). 빈값이면 미주입=codex 기본. 값이 있을 때만 `-c model_reasoning_effort=<값>` 플래그 부착. SKILL 리더 전용 |
