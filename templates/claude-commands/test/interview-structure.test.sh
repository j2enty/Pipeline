...
# ── (I-6) §3.6 사람용 문서 핵심 섹션 존재 ────────────────────────────────────
for section in "결정할 것" "안 하는 것" "키 피처" "리스크"; do
...
# ── (I-7) 접근법 슬롯 (3.5번 → 결정할 것) 존재 ───────────────────────────────
if grep -q "접근법" "$TEMPLATE" && grep -q "결정할 것" "$TEMPLATE"; then
  pass "(I-7) 접근법 결정 슬롯 (3.5번 → 결정할 것) 존재"
else
  fail "(I-7) 접근법 결정 슬롯 없음"
fi
