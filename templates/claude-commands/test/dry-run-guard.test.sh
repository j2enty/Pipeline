#!/usr/bin/env bash
# dry-run-guard.test.sh — plan.md.tmpl 안전 가드 정적 분석 테스트.
#
# 목적:
#   /plan 의 --dry-run 안전 모드가 "실제 GitHub 오염 없이" 테스트 가능하도록
#   plan.md.tmpl 에 박힌 가드들이 구조적으로 안전한지 정적으로 단언한다.
#
# 결정적·격리:
#   - placeholder 치환 전 원본 plan.md.tmpl 에 대해 그대로 돈다 (install 불필요).
...
#
# 왜 "코드펜스 안"으로 한정하나:
#   plan.md.tmpl 은 마크다운 지시문이라 원격 쓰기 명령의 이름이 산문/백틱 설명에도 등장한다.
...
# TEMPLATE 은 외부에서 주입 가능 — 음성 테스트가 임시 사본 경로를 넘길 때 사용
...
printf "\n${C_CYAN}══════════════════════════════${C_NC}\n"
if [ "$FAILED" -eq 0 ]; then
  printf "${C_GREEN}전체 PASS — dry-run 가드 안전 보장 확인${C_NC}\n"
  exit 0
else
